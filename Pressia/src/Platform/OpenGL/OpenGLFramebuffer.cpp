...
namespace Pressia {
	...
	namespace Utils {
		...
		static void AttachColorTexture(uint32_t id, int samples, GLenum internalFormat, GLenum format, uint32_t width, uint32_t height, int index) {
			...
			if (multisampled) {
				...
				//	TODO: Set with fbspec in future
	...
	void OpenGLFramebuffer::Invalidate() {
		...
		if (m_ColorAttachments.size() > 1) {	//	Multiple render targets
			PS_CORE_ASSERT(m_ColorAttachments.size() <= 4, "Framebuffer has 4 or more color attachments");
			GLenum buffers[4] = { GL_COLOR_ATTACHMENT0, GL_COLOR_ATTACHMENT1 ,GL_COLOR_ATTACHMENT2 ,GL_COLOR_ATTACHMENT3 };
			glDrawBuffers(m_ColorAttachments.size(), buffers);
		}
		else if (m_ColorAttachments.empty()) {
			glDrawBuffer(GL_NONE);	//	Only draw depth buffers
		}

		PS_CORE_ASSERT(glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE, "Framebuffer is incomplete");
